...
class UmrahGuideViewModel: ObservableObject {
    ...
    func saveSteps() {
        if let encodedData = try? JSONEncoder().encode(steps) {
            UserDefaults.standard.set(encodedData, forKey: "UmrahSteps")
        }
    }
    
    @Published var steps: [UmrahStep] = {
        if let savedData = UserDefaults.standard.data(forKey: "UmrahSteps"),
           let savedSteps = try? JSONDecoder().decode([UmrahStep].self, from: savedData) {
            return savedSteps
        } else {
            return [
                UmrahStep(
